...
internal static class PluginSettings
{
    ...
    internal static bool SubtextDebug => _subtextDebug?.Value ?? false;
    ...
    internal static void Init(ConfigFile config)
    {
        ...
        _labelsEnabled = config.Bind("Hooks", "LabelsEnabled", false,
            "Reservado — UiTextPatch desligado (v0.13.5). Nao patcha TMP/UI por performance.");
        ...
        _subtextDebug = config.Bind("Diagnostic", "SubtextDebug", true,
            "Lista campos testados ao capturar subtexto — etapa DebugSubtexto (so InteractionSubtextCapture).");
    }
}
...
// --- GameDataPaths.cs ---
/// <summary>
/// Pasta Translation/ do jogo (LocalLow / unity3d) — mesmo lugar do language pack CDN.
/// </summary>
internal static class GameDataPaths
{
    ...
    {
        foreach (var dir in CandidateTranslationDirs())
        {
            if (File.Exists(Path.Combine(dir, "version.json"))
                || File.Exists(Path.Combine(dir, NpcTalkStore.FileName))
                || File.Exists(Path.Combine(dir, NpcTalkStore.LegacyYamlName)))
                return dir;
        }
        ...
        return CandidateTranslationDirs()[0];
    }

    private static List<string> CandidateTranslationDirs()
    {
        var dirs = new List<string>();
        const string gameFolder = "Elder Game";
        const string projectFolder = "Project Gorgon";

        var appData = Environment.GetEnvironmentVariable("APPDATA");
        if (!string.IsNullOrEmpty(appData))
        {
            var localLow = Path.GetFullPath(Path.Combine(appData, "..", "LocalLow"));
            dirs.Add(Path.Combine(localLow, gameFolder, projectFolder, "Translation"));
        }
        ...
        if (!string.IsNullOrEmpty(localAppData))
        {
            ...
            var fromLocal = Path.Combine(localLow, gameFolder, projectFolder, "Translation");
            if (!dirs.Contains(fromLocal))
                dirs.Add(fromLocal);
        }
        ...
        {
            var linux = Path.Combine(
                home, ".config", "unity3d", gameFolder, projectFolder, "Translation");
            if (!dirs.Contains(linux))
                dirs.Add(linux);
        }
...
internal static class TalkLog
{
    ...
    /// <summary>Clique no NPC — sempre loga (independente de TalkVerbose).</summary>
    ...
    internal static void Subtexto(string text, string source)
    {
        ...
        LogAscii.LogInfo($"{Now()} {NpcPrefix()}etapa Subtexto ({source}): \"{text}\"");
    }

    /// <summary>Probe de candidatos a subtexto — so dentro de InteractionSubtextCapture.</summary>
    ...
    internal static void PhaseApplyOnly(LivePhase phase, IReadOnlyList<string> speeches, IReadOnlyList<string> choices)
    {
        ...
        switch (phase)
        {
            case LivePhase.DisplayTalkScreen:
                Pipeline(TalkPipelineStep.AplicarTela, "traducao em cache (sem Google)");
                break;
            case LivePhase.UpdateFancyMenuOptions:
                if (choices.Count > 0)
                    Pipeline(TalkPipelineStep.AplicarTela, "botoes em cache (sem Google)");
                break;
        }
    }
...
// --- NpcRegistryStore.cs ---
/// <summary>
/// Verifica NPC (nome + subtexto) em strings_npcs/requested e registra descobertas em strings_npcregistry.json.
/// </summary>
internal static class NpcRegistryStore
{
    internal const string FileName = "strings_npcregistry.json";
    ...
    internal static void Init(ConfigFile config)
    {
        _saveNew = config.Bind("NpcTalk", "SaveNewNpcRegistry", true,
            "Grava NPCs descobertos (nome + subtexto EN) em Translation/strings_npcregistry.json.");
        _path = Path.Combine(GameDataPaths.TranslationDir, FileName);
    ...
    internal static string ResolveSubtext(string npcId, string descEn)
    {
        ...
        if (NpcTalkStore.TryGet(descEn, out var cached, out _))
        {
            TalkLog.SubtextResolved(cached, "strings_npctalk.json");
            return cached;
        }

        if (CdnStringStore.TryGet(descEn, out cached, out var cdnSource))
        {
        ...
        NpcTalkStore.Record(descEn, translated[0]);
...
internal static class NpcTalkStore
{
    ...
    {
        using var scope = TraceLog.Scope("NpcTalkStore.MaybeMigrateLegacyYaml");
        var jsonPath = GameDataPaths.NpcTalkPath;
        var yamlPath = GameDataPaths.LegacyNpcYamlPath;

        if (!File.Exists(yamlPath))
        {
            TraceLog.Event("NpcTalkStore.MaybeMigrateLegacyYaml.skip noYaml");
            return;
        }

        var yamlEntries = LoadLegacyYaml(yamlPath);
        if (yamlEntries.Count == 0)
        {
            TraceLog.Event("NpcTalkStore.MaybeMigrateLegacyYaml.skip emptyYaml");
            return;
        }
        ...
        lock (Lock)
        {
            merged = File.Exists(jsonPath)
                ? LoadJsonFile(jsonPath)
                : new Dictionary<string, string>(StringComparer.Ordinal);

            var added = 0;
            foreach (var kv in yamlEntries)
            {
            ...
            if (added == 0 && File.Exists(jsonPath))
            {
                TraceLog.Event("NpcTalkStore.MaybeMigrateLegacyYaml.skip alreadyMerged");
                return;
            }

            WriteJsonFile(jsonPath, merged);
            _entries = merged;
        }

        TraceLog.Event($"NpcTalkStore.MaybeMigrateLegacyYaml migrated={yamlEntries.Count} path={jsonPath}");
        LogAscii.LogInfo(
            $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} NpcTalk: migrado {yamlEntries.Count} entradas de npcs.yaml → {FileName}");
    }
    ...
    private static Dictionary<string, string> LoadLegacyYaml(string path)
    {
        using var scope = TraceLog.Scope("NpcTalkStore.LoadLegacyYaml");
        var entries = new Dictionary<string, string>(StringComparer.Ordinal);
        if (!File.Exists(path))
            return entries;
        ...
        TraceLog.Event($"NpcTalkStore.LoadLegacyYaml done entries={entries.Count}");
        return entries;
    }

    private static (string Key, string Value)? SplitYamlKeyValue(string line)
    {
        var inQuotes = false;
        var escape = false;
...
internal static class TalkDynamicPatch
{
    ...
    internal static void ApplyTranslationPatches()
    {
        ...
        {
            if (IsTranslationPatched(method))
            {
...
internal static class TalkScreenPatch
{
    ...
    private static int PatchSessionHooks(Harmony harmony, Dictionary<string, List<MethodInfo>> map, Type controllerType)
    {
        ...
        var fancyMenu = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.UpdateFancyMenuPostfix));
        var fancyMenuOptions = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.UpdateFancyMenuOptionsPostfix));

        foreach (var method in ResolveMethods(map, SessionNpcNames))
        {
            harmony.Patch(method, prefix: npcCapture, postfix: npcCapturePost);
        ...
        {
            harmony.Patch(method, prefix: selectEntity, postfix: selectEntityPost);
        ...
        {
            harmony.Patch(method, prefix: open);
        ...
        count += PatchControllerPostfixes(harmony, controllerType, "UpdateFancyMenu", fancyMenu);
        count += PatchControllerPostfixes(harmony, controllerType, "UpdateFancyMenuOptions", fancyMenuOptions);
        count += InteractionClickPatch.PatchAll(harmony, controllerType);

        foreach (var method in ResolveMethods(map, SessionCloseNames))
        {
            harmony.Patch(method, postfix: close);
...
internal static class TextPatchHelper
{
    ...
    public static void InteractionPanelPostfix(object __instance)
    {
        TraceLog.Hook("InteractionPanelPostfix");
        UiInteractionFinder.Remember(__instance);
        InteractionSubtextCapture.TryCaptureFromClick(__instance?.GetType().Name ?? "InteractionPanel", __instance);
    }
    ...
    public static void UpdateInteractorNameStringPostfix(object __instance, object __0)
    {
        ...
        UiInteractionFinder.Remember(__instance);
        ApplyInteractorName(-1, Il2CppStringHelper.Read(__0));
        InteractionSubtextCapture.TryCaptureFromClick("UpdateInteractorName", __instance);
    }
    ...
    public static void UpdateInteractorNameEntityStringPostfix(object __instance, int __0, object __1)
    {
        ...
        TalkDiagnostics.HitUpdateInteractorName();
        UiInteractionFinder.Remember(__instance);
        ApplyInteractorName(__0, Il2CppStringHelper.Read(__1));
        InteractionSubtextCapture.TryCaptureFromClick("UpdateInteractorName", __instance);
    }

    private static void ApplyInteractorName(int entityId, string? name)
    {
        if (TalkSession.Active)
            return;
    ...
    public static void UpdateFancyMenuPostfix(object __instance)
    {
        TraceLog.Hook("UpdateFancyMenuPostfix");
        TalkDiagnostics.HitUpdateFancyMenu();
        UiInteractionFinder.Remember(__instance);
        InteractionSubtextCapture.TryCaptureFromClick("UpdateFancyMenu", __instance);
        PreTalkScreenProbe.CaptureFromInteractionPanel(__instance, logInteraction: false);
    }

    public static void UpdateFancyMenuOptionsPostfix(object __instance)
    {
        TraceLog.Hook("UpdateFancyMenuOptionsPostfix");
        TalkDiagnostics.HitUpdateFancyMenuOptions();
        UiInteractionFinder.Remember(__instance);
        InteractionSubtextCapture.TryCaptureFromClick("UpdateFancyMenuOptions", __instance);
        if (TalkSession.InteractionLogged)
            return;
    ...
    public static void OnPreTalkPostfix(object __instance, object info)
    {
        ...
        UiInteractionFinder.Remember(__instance);
        NpcIdDebugProbe.ScanHook("OnPreTalk", null, info);
        InteractionSubtextCapture.TryCaptureFromClick("OnPreTalk", __instance);
...
/// <summary>
/// Clique no NPC — UISelectionController (base) + UIInteractionController (derivada).
/// </summary>
internal static class InteractionClickPatch
{
    private const string SelectionControllerTypeName = "UISelectionController";

    private static readonly string[] SelectionHookNames =
    {
        "OnNewTarget",
        "InvokeOnNewTarget",
    ...
    private static readonly string[] ControllerPanelHookNames =
    {
        "UpdateInteractionPanel",
        "UpdateInteractorDesc",
        "UpdateInteractorSubtext",
    ...
    {
        var count = 0;
        var selectionPostfix = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.UiSelectionPostfix));
        var nameStringPostfix = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.UpdateInteractorNameStringPostfix));
        var nameEntityStringPostfix = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.UpdateInteractorNameEntityStringPostfix));
        var panelPostfix = new HarmonyMethod(typeof(TextPatchHelper), nameof(TextPatchHelper.InteractionPanelPostfix));

        var selectionController = ResolveType(SelectionControllerTypeName);
        if (selectionController == null)
            LogAscii.LogWarning("  UISelectionController nao encontrado — OnNewTarget sem hook");

        foreach (var type in EnumerateTypes(selectionController, interactionControllerType))
        {
            foreach (var methodName in SelectionHookNames)
                count += PatchPostfix(harmony, type, methodName, selectionPostfix, requireSelectionArg: true);
        }

        count += PatchAllOverloads(harmony, interactionControllerType, "UpdateInteractorName", panelPostfix);
        count += PatchPostfix(harmony, interactionControllerType, "UpdateInteractorName", nameStringPostfix, paramCount: 1);
        count += PatchPostfix(harmony, interactionControllerType, "UpdateInteractorName", nameEntityStringPostfix, paramCount: 2);

        if (selectionController != null && !ReferenceEquals(selectionController, interactionControllerType))
        {
            count += PatchAllOverloads(harmony, selectionController, "UpdateInteractorName", panelPostfix);
            count += PatchPostfix(harmony, selectionController, "UpdateInteractorName", nameStringPostfix, paramCount: 1);
            count += PatchPostfix(harmony, selectionController, "UpdateInteractorName", nameEntityStringPostfix, paramCount: 2);
        }

        foreach (var methodName in ControllerPanelHookNames)
            count += PatchAllOverloads(harmony, interactionControllerType, methodName, panelPostfix);

        if (count == 0)
            LogAscii.LogWarning("  nenhum hook de clique NPC aplicado");

        count += SelectionDebugProbe.PatchDebugHooks(harmony, selectionController);

        return count;
    }

    private static IEnumerable<Type> EnumerateTypes(Type? selectionController, Type interactionControllerType)
    {
        if (selectionController != null)
            yield return selectionController;

        if (interactionControllerType != null)
            yield return interactionControllerType;
    }
    ...
    private static int PatchPostfix(
        Harmony harmony,
        Type? type,
        string methodName,
        HarmonyMethod postfix,
        bool requireSelectionArg = false,
        int paramCount = -1)
    {
        if (type == null)
            return 0;
        ...
        foreach (var method in type.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
        {
            ...
            if (requireSelectionArg && !TakesSelectionArg(method))
                continue;

            if (paramCount >= 0 && method.GetParameters().Length != paramCount)
                continue;
            ...
            {
                harmony.Patch(method, postfix: postfix);
                TraceLog.Event($"Harmony.Patch.click {type.FullName}.{method.Name}");
                LogAscii.LogInfo($"  click {type.Name}.{method.Name}");
                count++;
            }
            catch (Exception ex)
            {
                TraceLog.Event($"Harmony.Patch.skip {type.FullName}.{method.Name} {ex.GetType().Name}: {ex.Message}");
                LogAscii.LogWarning($"  click skip {type.Name}.{method.Name}: {ex.GetType().Name}");
            }
        }
        ...
        HarmonyMethod postfix)
    {
        ...
        foreach (var method in type.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
        {
            ...
            {
                harmony.Patch(method, postfix: postfix);
                TraceLog.Event($"Harmony.Patch.panel {type.FullName}.{method.Name}");
                LogAscii.LogInfo($"  panel {type.Name}.{method.Name}");
                count++;
            }
            catch (Exception ex)
            {
                TraceLog.Event($"Harmony.Patch.skip {type.FullName}.{method.Name} {ex.GetType().Name}: {ex.Message}");
                LogAscii.LogWarning($"  panel skip {type.Name}.{method.Name}: {ex.GetType().Name}");
            }
        }
    ...
    private static bool TakesSelectionArg(MethodInfo method)
    {
        ...
        return paramType.Name.Contains("UISelection", StringComparison.Ordinal)
               || paramType.Name.Contains("Selection", StringComparison.Ordinal);
    }
}
...
// --- UiTextPatch.cs ---
/// <summary>
/// Rotulos do mundo (Lamp, etc.) — desligado em v0.13.5.
/// Hooks globais em TMP_Text/UI.Text geravam lookup CDN a cada frame (ex.: "(None)")
/// e nao corrigiam nameplates 3D no IL2CPP.
/// </summary>
internal static class UiTextPatch
{
    internal static int PatchAll(Harmony harmony)
    {
        TraceLog.Event("UiTextPatch.disabled v0.13.5");
        LogAscii.LogInfo("  UiTextPatch desligado — busca CDN em rotulos UI removida (performance).");
        return 0;
    }
}
...
#region Plugin
// --- Plugin.cs ---
[BepInPlugin("com.pg.translatelive", "Pg Translate Live", "0.14.30")]
public class Plugin : BasePlugin
{
    ...
    public override void Load()
    {
        ...
        if (PluginSettings.UltraShellMode)
        {
            ...
            LogAscii.LogInfo($"{System.DateTime.Now:yyyy-MM-dd HH:mm:ss} Pg Translate Live v0.14.30 - ULTRA-SHELL");
            return;
        }
        ...
        LogAscii.LogInfo($"{System.DateTime.Now:yyyy-MM-dd HH:mm:ss} Pg Translate Live v0.14.30 - {mode}");
