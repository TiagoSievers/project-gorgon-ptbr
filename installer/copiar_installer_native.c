...
static BOOL validate_pack(const WCHAR *root, WCHAR *missing, DWORD missing_chars) {
    ...
    join_path(path, 32768, root, L"para-pasta-do-jogo\\BepInEx\\plugins\\PgTranslateLive\\PgTranslateLive.dll");
    if (!file_exists(path)) {
        _snwprintf(missing, missing_chars, L"para-pasta-do-jogo\\BepInEx\\plugins\\PgTranslateLive\\PgTranslateLive.dll");
        return FALSE;
    }

    join_path(path, 32768, root, L"para-Translation\\version.json");
    if (!file_exists(path)) {
        _snwprintf(missing, missing_chars, L"para-Translation\\version.json");
        return FALSE;
    }
...
static BOOL install_game_files(const WCHAR *root, const WCHAR *game_dir) {
    WCHAR src[32768];
    join_path(src, 32768, root, L"para-pasta-do-jogo");
    if (!copy_tree(src, game_dir)) {
        log_append(L"ERRO: falha ao copiar para-pasta-do-jogo");
        return FALSE;
    }
    log_append(L"BepInEx + plugin copiados para pasta do jogo");
    return TRUE;
}

static void remove_legacy_translator(const WCHAR *game_dir) {
    WCHAR legacy[32768];
    join_path(legacy, 32768, game_dir, L"BepInEx\\plugins\\Translator");
    if (dir_exists(legacy)) {
        remove_tree(legacy);
        RemoveDirectoryW(legacy);
        log_append(L"Removido legado: BepInEx\\plugins\\Translator");
    }
}
...
static BOOL install_translation(const WCHAR *root) {
    ...
    join_path(src, 32768, root, L"para-Translation");
    ...
    log_append(L"Language pack: %ls", dest);
    return TRUE;
}
...
static BOOL verify_install(const WCHAR *game_dir) {
    ...
    if (!file_exists(check)) {
        log_append(L"ERRO: version.json ausente em Translation/");
        return FALSE;
    }
...
static BOOL install_all(const WCHAR *root, const WCHAR *game_dir, ProgressFn progress) {
    ...
    prog(30, L"Copiando BepInEx + plugin...");
    if (!install_game_files(root, game_dir)) {
        return FALSE;
    }

    prog(50, L"Removendo mod legado...");
    remove_legacy_translator(game_dir);
...
static void ui_create_controls(InstallerUi *ui) {
    ...
    HWND intro = CreateWindowW(
        L"STATIC",
        L"Instalando PT-BR (copia para pasta do jogo + Translation)...",
        WS_CHILD | WS_VISIBLE,
        12, 12, 680, 22,
        hwnd, NULL, NULL, NULL
    );
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE prev, PWSTR cmd, int show) {
    ...
    _snwprintf(
        msg,
        4096,
        L"O instalador vai copiar para-pasta-do-jogo e para-Translation para:\n\n%ls\n\n"
        L"e para AppData\\LocalLow\\...\\Translation\\\n\n"
        L"Depois disso, uma janela vai mostrar o progresso.",
        game_dir
    );
    msg[4095] = L'\0';
